...
var themeFunctionality = {
	...
	jsSFNT: function() {
		...
		function brandsAjax(e){
			...
			var smoothRemove = function(slider, response) {
				...
				addSlides(slider, response);
				
			}

			// var smoothAdd = function(slider, newSlides) {
			...
			// 	// 	slider.slick( 'slickAdd', $(newSlides[i]) );

			// 	// }
				
			// }

			var removeSlides = function( slider, callback, response ) {

				var slider = $( slider );
				
				if ( slider && slider[0].slick.$slides && slider[0].slick.$slides.length > 0 ) {
			...
			var addSlides = function( slider, html, callback ) {
			...
			// window.testRemoveSlides = removeSlides;
			// window.testSlider = brandsContainer;

			// brandsLoadingSlider = brandsContainer.querySelectorAll('.slick-slide');
			// for (i = 0; i < brandsLoadingSlider.length; i++) {
			  
			//   console.log(brandsLoadingSlider[i]);
			//   $('#x-carousel-featured-brand').slick('slickRemove', 0);
			// }
			...
			request.onreadystatechange = function () {
				...
				if (request.status === 200) {
					...
					removeSlides( brandsContainer, smoothRemove, response );
